...
module Types = struct
  ...
  let add ?(recursive=false) env vars id body loc =
    ...
    | Algebraic l
      ...
      in
      let ty = Ty.tadt ty_vars id l in
      ty, { env with to_ty = MString.add id ty env.to_ty }
...
module Env = struct
  ...
  let add_type_decl ?(recursive=false) env vars id body loc =
    let ty, types = Types.add ~recursive env.types vars id body loc in
          ...
          let acc_z, env_z
            ...
            List.fold_left
              (fun (acc_x, env_x) (cstr, lbl_args_ty) ->
                 ...
                 in
                 let td2 = TLogic(loc, [cstr], tty) in
                 let td2_a = { c = td2; annot=new_id () } in
        ...
        (fun (acc, env1) (loc, ls, s, body) ->
           ...
           begin match body with
             | Enum lc ->
               let lcl = List.map (fun c -> c, "") lc in (* TODO change this *)
               let ty = PFunction([], pur_ty) in
               let tty, env2 = Env.add_logics env1 Symbols.constr lcl ty loc in
               let td2 = TLogic(loc, lc, tty) in
               let td2_a = { c = td2; annot=new_id () } in
               (td2_a,env2)::acc, env2

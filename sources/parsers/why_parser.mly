...
| MATCH e = lexpr WITH cases = list1_match_cases END
    { mk_match ($startpos, $endpos) e cases }


list1_match_cases:
|     p = simple_pattern RIGHTARROW e = lexpr { [p, e]}
| BAR p = simple_pattern RIGHTARROW e = lexpr { [p, e]}
| list1_match_cases BAR p = simple_pattern RIGHTARROW e = lexpr { [p, e]}

simple_pattern:
| id = ident { mk_application ($startpos, $endpos) id [] }
| app = ident LEFTPAR args = list0_ident_sep_comma RIGHTPAR
...
| se = simple_expr SHARP label = ident
   { mk_algebraic_select ($startpos, $endpos) ~guarded:true se label }
array_assignements:
...
|  { [] }
| id = ident
    { [ mk_application ($startpos, $endpos) id [] ] }

| id = ident COMMA l = list0_ident_sep_comma
    { (mk_application ($startpos, $endpos) id []) :: l }
